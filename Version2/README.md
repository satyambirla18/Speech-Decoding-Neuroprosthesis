...
- `--grad-checkpoint` to enable activation checkpointing inside Conformer blocks
- `--use-sdpa / --no-use-sdpa` to toggle PyTorch scaled-dot-product-attention backends (Flash/mem-efficient)
- `--optimizer adamw|adamw8bit` (8-bit requires `bitsandbytes`)
- `--grad-accum-steps N` for gradient accumulation
- `--use-conv / --no-use-conv` to toggle Conformer convolution module
- `--compile` to enable `torch.compile`
- `--tf32 / --no-tf32` to toggle TF32 matmul/cudnn
