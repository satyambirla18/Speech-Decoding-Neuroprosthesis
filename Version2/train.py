...
def main():
    ...
    p.add_argument("--conv-kernel-size", type=int, default=31)
    p.add_argument("--use-conv", action=argparse.BooleanOptionalAction, default=True)
    ...
    p.add_argument("--grad-checkpoint", action="store_true", help="Enable activation checkpointing inside Conformer blocks.")
    p.add_argument("--amp", type=str, choices=["bf16","fp16","off"], default="bf16", help="Automatic mixed precision mode.")
    p.add_argument("--grad-accum-steps", type=int, default=1, help="Gradient accumulation steps.")
    p.add_argument("--max-grad-norm", type=float, default=1.0)
    p.add_argument("--optimizer", type=str, choices=["adamw","adamw8bit"], default="adamw")
    p.add_argument("--tf32", action=argparse.BooleanOptionalAction, default=True, help="Enable TF32 matmul/cudnn for speed on Ampere+.")
    p.add_argument("--compile", action="store_true", help="Use torch.compile (optional, may increase compile time).")
    ...
    if args.optimizer == "adamw8bit":
        try:
            ...
            print("WARNING: --optimizer adamw8bit requested but bitsandbytes not available; falling back to AdamW. Error:", e)
